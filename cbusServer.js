	...
	for (x in dataarray) { // loop through array of can frames
		...
		if (frame.length > 0) {
			...
			if (opc==98||opc==99||opc==90||opc==91) { // hex opc values
				if (opc==98) { newOutPut+= "Short Event On " }
				if (opc==99) { newOutPut+= "Short Event Off " }
				if (opc==90) { newOutPut+= "Long Event On " }
				if (opc==91) { newOutPut+= "Long Event Off " }
...
var server = net.createServer(function (socket) {
    ...
    socket.on('data', function (data) {
        ...
        if (DECIMALEVENTS==1) {
			console.log(relativeTime() + socket.localAddress + " " + data.toString() );
			displayDataString(data);
		} else {
